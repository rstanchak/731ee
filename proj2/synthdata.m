...
len = length(x);
half = length(x)/2;
T = [x(half+1:len) reverse(x(half+1:len)) ; ...
      zeros(1,len); ...
      y(half+1:len) -reverse(y(half+1:len)) ];
...
U = R*S + gaussnoise(size(R,1), size(S,2), var);
...
plot(1:length(yaw3), yaw3, 'y');
plot(1:length(yaw1), yaw1, 'r');
subplot(312)
...
plot(1:length(pitch3), pitch3, 'y');
plot(1:length(pitch1), pitch1, 'r');
subplot(313)
...
plot(1:length(roll3), roll3, 'y');
plot(1:length(roll1), roll1, 'r');
