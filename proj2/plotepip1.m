function plotepip(f1,f2,p1,p2,F)
...
for i=1:size(p1,1)
	% plot on figure 1
	figure(f1);
	[pt1, pt2] = clipline(l1(i,1), l1(i,2), l1(i,3), axis());
	line( [pt1(:,1); pt2(:,1)], [pt1(:,2); pt2(:,2)] );
	hold on; plot( p1(i,1), p1(i,2), 'r+' ); hold off;
	pt1
	pt2

	% plot on figure 2 
	figure(f2); 
	[pt1, pt2] = clipline(l2(i,1), l2(i,2), l2(i,3), axis());
	%l2(1:10,:)
	%pt1(1:10,:)
		%pt2(1:10,:)
	%p2(1:10,:)
	line( [pt1(:,1); pt2(:,1)], [pt1(:,2); pt2(:,2)] );
	hold on; plot( p2(i,1), p2(i,2), 'r+' ); hold off;

	point_line_dist( l1(i,:), p1(i,:) )
	point_line_dist( l2(i,:), p2(i,:) )
end
